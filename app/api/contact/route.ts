...
export async function POST(req: Request) {
  try {
    ...
    const clientRequestId = req.headers.get("x-client-request-id") ?? "unknown";
    ...
    console.info("[contact] received", { requestId, clientRequestId });

    const turnstileResult = await verifyTurnstile(payload.turnstileToken, ip);
    if (!turnstileResult.success) {
      console.warn("[contact] turnstile failed", { requestId, clientRequestId, detail: turnstileResult.error });
      return NextResponse.json(
        { ok: false, error: "turnstile_failed", detail: turnstileResult.error },
        { status: 400 },
      );
    }
    console.info("[contact] turnstile ok", { requestId, clientRequestId });

    const connectionString = process.env.DATABASE_URL;
    if (!connectionString) {
      console.error("[contact] database unavailable", { requestId, clientRequestId });
    ...
    try {
      ...
      console.info("[contact] saved", { requestId, clientRequestId });
    } finally {
      await client.end();
    }

    await sendEmail(payload);
    console.info("[contact] email sent", { requestId, clientRequestId });
